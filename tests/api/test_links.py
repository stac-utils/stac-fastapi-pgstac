...
@pytest.mark.parametrize(
    "malicious_host",
    [
        "evil.com/admin",  # path injection -> request.url.path divergence
        "evil.com:8080/x",  # port + path injection
        "stac.io@evil.com",  # userinfo trick
        "evil com",  # invalid character (space)
    ],
)
def test_malformed_host_header_not_reflected_in_links(malicious_host):
    """Regression test for CVE-2026-48710.

    A malformed HTTP ``Host`` header must not be used to reconstruct
    ``request.url``. Starlette >= 1.0.1 validates the header and falls back to
    the ASGI ``server`` value for malformed input. Before the fix a malformed
    host such as ``evil.com/admin`` poisoned ``request.url`` (and therefore
    every generated link) and even made ``request.url.path`` diverge from the
    routed path, allowing security checks keyed on ``request.url`` to be
    bypassed.
    """
    app = FastAPI()
    router = APIRouter()
    ...
    with TestClient(app, base_url="http://stac.io") as client:
        response = client.get("/search", headers={"host": malicious_host})
        assert response.status_code == 200

        # The routed path must not be influenced by the Host header.
        assert response.json()["path"] == "/search"

        # No generated link may leak the spoofed host or an injected path.
        for link in response.json()["links"]:
            assert link["href"].startswith("http://stac.io/")
            assert "evil" not in link["href"]
            assert "/admin" not in link["href"]
