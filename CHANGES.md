...
- Fix `GET /collections` returning a 500 error when the `query` parameter is used: the injected `type = Collection` constraint is now merged into whichever of `query`/`filter` the client supplied, since pgstac rejects a `query` + `filter` payload
- Return `400` instead of a pgstac error when `query` and `filter` are used together in `GET /collections` requests
